// moderation-bot.js
// Persistent auto-update via version.txt like premade bot
...
const VERSION_FILE = "/mnt/server/version.txt";
const VERSIONS_URL =
...
// Read local version
let localVer = "0.0.0";
if (fs.existsSync(VERSION_FILE)) localVer = fs.readFileSync(VERSION_FILE, "utf8").trim();
      ...
      .get(VERSIONS_URL, (res) => {
        ...
        res.on("end", () => {
          ...
          for (const line of lines) {
            if (line.startsWith(BOT_TYPE)) return resolve(line.split("==")[1].trim());
          }
          resolve(null);
        });
      })
    ...
    https
      .get(UPDATE_URL, (res) => {
        ...
        res.on("end", () => {
          ...
          console.log(`⬆️ Updated bot.js to version ${newVer}. Restart required.`);
          process.exit(0);
        });
      })
      .on("error", () => resolve());
    ...
    switch (interaction.commandName) {
      ...
      case "kick":
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.KickMembers))
          return interaction.reply({ content: "❌ You don’t got perms.", flags: MessageFlags.Ephemeral });
